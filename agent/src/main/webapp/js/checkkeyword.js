	...
	$(".checkselect").change(function(){
		...
		if(v == 2){
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为审核中吗？")){
				$.post('/updatekeyword',{'id':kid,'checkstatus':2},function(result){
					if("success" == result){
			...
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为审核通过吗？")){
				$.post('updatekeyword.action',{'id':kid,'checkstatus':3},function(result){
					if("success" == result){
			...
			if(confirm("您确定要将["+obj.attr("keyword")+"]的状态修改为不通过吗？")){
				$.post('updatekeyword.action',{'id':kid,'checkstatus':4},function(result){
					if("success" == result){
			...
			if(confirm("您确定要对["+obj.attr("keyword")+"]进行续费操作吗？")){
					ymPrompt.win({message:'/xufei?id='+kid,width:600,height:400,title:'当前为【'+obj.attr("keyword")+'】进行续费操作',handler:callBack,iframe:true})
				}
		}else if(v == 6){
			if(confirm("您确定要修改["+obj.attr("keyword")+"]的使用状态为不使用吗？")){
				$.post('updatekeyword.action',{'id':kid,'isuse':1},function(result){
					if("success" == result){
			...
			if(confirm("您确定要修改["+obj.attr("keyword")+"]的使用状态为使用吗？")){
				$.post('updatekeyword.action',{'id':kid,'isuse':2},function(result){
					if("success" == result){
