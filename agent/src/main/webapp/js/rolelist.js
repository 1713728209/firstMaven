$(document).ready(function(){
	...
	$("#addRoleSubmit").click(function(){
		...
		if(a_roleName == ''){
			...
			$.post("/editrole",{'type':'add','rolename':a_roleName,'isstart':a_isStart},function(result){
				if("success" == result){
					alert("添加成功！");
	...
	$("#modifyRoleSubmit").click(function(){
		...
		if(m_roleName == ''){
			...
			$.post("/editrole",{'type':'modify','id':m_roleId,'rolename':m_roleName,'isstart':m_isStart},function(result){
				if("success" == result){
					alert("修改成功！");
	...
	$(".deleteRole").click(function(){
		...
		if(confirm("您确定要删除【"+d_roleName+"】这个角色吗？")){
			//delete
			$.post("/delrole",{'id':d_roleId},function(result){
				if("success" == result){
					alert("删除成功！");
					window.location.href="rolelist.action";
